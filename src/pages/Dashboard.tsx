...
const Dashboard = () => {
  ...
  if (viewLaptops) {
    ...
    return (
      <div id="print-container" className="dashboard-print-page p-6 space-y-6 bg-gradient-to-br from-background via-background to-primary/5 dark:from-slate-900 dark:to-slate-800">
        <div className="dashboard-print-header flex items-center justify-between">
          <div className="flex items-start gap-3">
            <Button
              ...
              className="border border-primary/60 dark:border-primary/60 text-primary dark:text-primary bg-transparent hover:bg-primary/10 dark:hover:bg-primary/20 hover:text-primary hover:border-primary hover:ring-2 hover:ring-primary/30 transition-all"
  ...
  if (viewExpiredAntivirus) {
    ...
    return (
      <div id="print-container" className="dashboard-print-page p-6 space-y-6 bg-gradient-to-br from-background via-background to-primary/5 dark:from-slate-900 dark:to-slate-800">
        ...
        {/* Search & Filters */}
        <div className="no-print flex flex-col md:flex-row gap-3 items-stretch md:items-center bg-white/70 dark:bg-slate-800/60 backdrop-blur-sm border border-primary/30 dark:border-primary/40 rounded-xl p-3">
          <Input
        ...
        {assetsFiltered.length > 0 && (
          <div>
            <h2 className="text-2xl font-bold mb-4 text-primary">Expired IT Assets</h2>
                ...
                <div key={`a-${unit}`} className="mb-6">
                  {unit !== 'Unassigned' && (
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h3 className="text-lg font-semibold text-primary">{unit}</h3>
  ...
  if (!selectedUnit && !selectedDepartment) {
    ...
    return (
      <div className="p-6 space-y-6 bg-gradient-to-br from-background via-background to-primary/5 dark:from-slate-900 dark:to-slate-800">
        {/* Unit/Office Cards */}
        <div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
            <h2 className="text-2xl font-bold text-primary whitespace-nowrap">Units & Offices</h2>
            <SearchFilter
                  ...
                  <div className="space-y-4">
                    {list.map((g, gi) => (
                      <div key={gi} className="mb-1">
                        <div className="inline-flex items-center gap-2 mb-2 px-3 py-2 rounded-lg bg-primary/10 border border-primary/30 w-auto max-w-full">
                          <Building2 className="h-4 w-4 text-primary flex-shrink-0" />
                          <span className="text-sm font-bold text-primary">{g.unit}</span>
                          <span className="text-primary/60">/</span>
                          <span className="text-sm font-semibold text-primary/80">{g.dept}</span>
                        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(q ? [] : unitsToShow).map((unit, index) => (
              <Card
                key={unit.id}
                className={`cursor-pointer dashboard-card-entry ${index % 2 === 0 ? 'dashboard-card-entry-left' : 'dashboard-card-entry-right'} bg-transparent border border-primary/40 hover:border-primary transition-all duration-300`}
                style={{ animationDelay: `${index * 120}ms` }}
                onClick={() => handleUnitClick(unit)}
              >
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-primary text-xs sm:text-lg min-w-0">
                    <img
                      src="/pictures/20eb7d56-b963-4a41-9830-eead460b0120.png"
                      alt="MNR Group Logo"
                      className="w-8 h-8 sm:w-12 sm:h-12 flex-shrink-0 rounded-full object-contain bg-white p-1 border border-primary/30 shadow-sm"
                    />
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4 text-center">
                    <div>
                      <div className="text-2xl font-bold text-primary">{unit.total_departments}</div>
                      <div className="text-xs text-muted-foreground">Departments</div>
                    </div>
                    <div>
                      <div className="text-2xl font-bold text-primary/90">{unit.total_assets}</div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {/* Total Assets Card */}
            <Card
              className="relative overflow-hidden cursor-pointer border-2 border-primary/40 bg-transparent shadow-lg dashboard-card-entry dashboard-card-entry-top hover:shadow-[0_0_0_1px_hsl(var(--primary)/0.14),0_12px_30px_hsl(var(--primary)/0.18)] hover:border-primary hover:shadow-primary/30 transition-all duration-500 group"
            ...
            {/* Laptops Card */}
            <Card
              className="relative overflow-hidden cursor-pointer border-2 border-primary/40 bg-transparent shadow-lg dashboard-card-entry dashboard-card-entry-right hover:shadow-[0_0_0_1px_hsl(var(--primary)/0.14),0_12px_30px_hsl(var(--primary)/0.18)] hover:border-primary hover:shadow-primary/30 transition-all duration-500 group"
            ...
            {/* Desktops Card */}
            <Card
              className="relative overflow-hidden cursor-pointer border-2 border-primary/40 bg-transparent shadow-lg dashboard-card-entry dashboard-card-entry-bottom hover:shadow-[0_0_0_1px_hsl(var(--primary)/0.14),0_12px_30px_hsl(var(--primary)/0.18)] hover:border-primary hover:shadow-primary/30 transition-all duration-500 group"
            ...
            {/* Antivirus Card */}
            <Card
              className={`relative overflow-hidden cursor-pointer border-2 bg-transparent shadow-lg dashboard-card-entry dashboard-card-entry-left hover:shadow-2xl transition-all duration-500 group ${stats.expiredAntivirus > 0 ? 'border-destructive/50 hover:border-destructive hover:shadow-destructive/30' : 'border-primary/40 hover:border-primary hover:shadow-[0_0_0_1px_hsl(var(--primary)/0.14),0_12px_30px_hsl(var(--primary)/0.18)]'}`}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-start gap-3">
            <Button
              ...
              className="border border-primary/40 text-primary bg-transparent hover:bg-primary/10 hover:text-primary hover:border-primary hover:ring-2 hover:ring-primary/30 transition-all"
              aria-label="Back to Units"
            >
              <ArrowLeft className="h-5 w-5" />
            </Button>
            <div>
              <h1 className="text-xs sm:text-3xl font-bold text-primary">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredDepartments.map((department, index) => (
              ...
              >
                <div className="relative p-5 text-foreground">
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-14 h-14 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300 text-primary">
                      <Building2 className="h-7 w-7" />
                    </div>
                    <div className="text-right">
                      <div className="text-4xl font-black text-primary">{department.total_assets}</div>
                      <div className="text-xs font-medium text-muted-foreground">Assets</div>
                    </div>
                  </div>
                  <h3 className="text-[11px] sm:text-lg font-bold truncate text-foreground">{department.name}</h3>
