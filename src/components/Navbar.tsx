...
import { Menu, X, LogOut, Palette, Check, Moon, Sun, BadgeCheck, Briefcase, Building2, MapPin, Bell, PlusCircle, Edit3, Trash, Trash2, X as XIcon, Database, Download, Upload, Shield, Monitor, AppWindow } from "lucide-react";
...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { path: "https://mnrslit.netlify.app/", label: "IT App", icon: AppWindow, external: true },
  ].filter((item) => {
    if (!access) return true;
    if (access.is_super_admin) return true;
    return access.allowed_routes?.includes(item.path);
  });

  return (
    <nav className="bg-primary/5 dark:bg-primary/10 backdrop-blur-md supports-[backdrop-filter]:bg-primary/5 dark:supports-[backdrop-filter]:bg-primary/10 border-b-2 border-primary/60 dark:border-primary/70 sticky top-0 z-50 shadow-sm">
        ...
        <div className="flex justify-between h-14">
          <div className="flex items-center">
            <Link to="/" className="flex items-center space-x-2">
              ...
              <div className="flex flex-col">
                <span className="font-semibold text-base text-foreground tracking-wide">MNR Group IT Support</span>
              </div>
            </Link>
          </div>
          ...
          <div className="flex items-center gap-2">
              ...
              {navItems.map((item) => {
                ...
                if (item.external) {
                  return (
                    <a
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-sm font-medium border border-transparent transition-all duration-200 text-foreground/70 hover:text-primary hover:bg-primary/10 hover:border-primary/30 hover:shadow-[0_0_0_1px_hsl(var(--primary)/0.08)]"
                    >
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-sm font-medium border transition-all duration-200 ${isActive
                      ? "bg-primary/10 text-primary border-primary/40 shadow-[0_0_0_1px_hsl(var(--primary)/0.08)]"
                      : "text-foreground/70 border-transparent hover:text-primary hover:bg-primary/10 hover:border-primary/30 hover:shadow-[0_0_0_1px_hsl(var(--primary)/0.08)]"
                      }`}
                  >
                    <Icon className="w-4 h-4" />
                    <span>{item.label}</span>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" sideOffset={8} className="w-[calc(100vw-1rem)] sm:w-72 border-2 border-primary/60 rounded-2xl shadow-[0_0_40px_-8px_hsl(var(--primary)/0.5)] bg-gradient-to-br from-background via-background to-primary/5 p-2">
                <div className="flex items-center justify-between gap-2 rounded-lg px-2 py-2 mb-2 bg-muted/50">
                ...
                <DropdownMenuItem onClick={toggleDark} className="gap-2 cursor-pointer rounded-lg">
      ...
      {isMobileMenuOpen && (
        <div className="border-t border-primary/60 bg-background max-h-[calc(100vh-3.5rem)] overflow-y-auto">
              ...
              return (
                <Link
                  ...
                  className={`flex items-center space-x-2 px-3 py-2 rounded-lg text-base font-medium ${isActive
                    ? "bg-primary/10 text-primary border border-primary/40 shadow-[0_0_0_1px_hsl(var(--primary)/0.08)]"
