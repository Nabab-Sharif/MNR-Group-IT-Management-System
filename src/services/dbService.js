...
class DBService {
  ...
  async exportData() {
    const data = {
      ...
      it_assets: await this.getITAssets(),
  ...
  normalizeImportData(data) {
    ...
    if (Array.isArray(data)) {
      return { assets: data, it_assets: data };
    }

    const normalized = { ...data };
    const nestedData = normalized.data && typeof normalized.data === 'object' ? normalized.data : {};

    const setAlias = (key, alias) => {
      if (Array.isArray(normalized[key]) && !Array.isArray(normalized[alias])) {
        normalized[alias] = normalized[key];
      }
      if (Array.isArray(normalized[alias]) && !Array.isArray(normalized[key])) {
        normalized[key] = normalized[alias];
      }
    };

    setAlias('assets', 'it_assets');
    setAlias('users', 'user');
    setAlias('departments', 'department');
    setAlias('units', 'unit');
    setAlias('products', 'product');
    ...
    if (normalized.assets === undefined && Array.isArray(normalized.it_assets)) {
      normalized.assets = normalized.it_assets;
    }
    if (normalized.it_assets === undefined && Array.isArray(normalized.assets)) {
      normalized.it_assets = normalized.assets;
    }
  ...
  async importData(data) {
    try {
      ...
      if (normalized.users) await indexedDB.bulkPut('users', normalized.users);
      if (normalized.departments) await indexedDB.bulkPut('departments', normalized.departments);
      if (normalized.assets || normalized.it_assets) await indexedDB.bulkPut('it_assets', normalized.assets || normalized.it_assets);
      if (normalized.units) await indexedDB.bulkPut('units', normalized.units);
      if (normalized.products) await indexedDB.bulkPut('products', normalized.products);
      if (normalized.activities) await indexedDB.bulkPut('user_activities', normalized.activities);
      if (normalized.printers) await indexedDB.bulkPut('printers', normalized.printers);
      if (normalized.ip_phones) await indexedDB.bulkPut('ip_phones', normalized.ip_phones);
      if (normalized.wifi_networks) await indexedDB.bulkPut('wifi_networks', normalized.wifi_networks);
      if (normalized.ip_addresses) await indexedDB.bulkPut('ip_addresses', normalized.ip_addresses);
      if (normalized.cctv_cameras) await indexedDB.bulkPut('cctv_cameras', normalized.cctv_cameras);
      if (normalized.nvrs) await indexedDB.bulkPut('nvrs', normalized.nvrs);
      if (normalized.cctv_checklists) await indexedDB.bulkPut('cctv_checklists', normalized.cctv_checklists);
      return true;
    } catch (error) {
